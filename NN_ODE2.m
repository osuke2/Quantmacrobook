...
function [Psi, dPsi] = forward_pass(x, v, theta, w, A)
    ...
    x_broadcast = repmat(x, 1, N);      % [1 × N]
    v_broadcast = repmat(v', n_points, 1);  % [n_points × 1]
    theta_broadcast = repmat(theta', n_points, 1);  % [n_points × 1]
    
    % Hidden layer computations
    z = v_broadcast .* x_broadcast + theta_broadcast;  % [n_points × N]
    y = tanh_activate(z);  % [n_points × N]
    
    % Derivative computations
    dydx = v_broadcast .* tanh_derivative(z);  % [n_points × N]
...
function y = tanh_activate(x)
    y = tanh(x);
end

function dy = tanh_derivative(x)
    s = tanh(x);
    dy = 1 - s.^2; 
end