...
v2 = randn(N2, N) * sqrt(2/N);  % Input weights
theta = randn(N, 1) * sqrt(2/1);  % Hidden biases
theta2 = randn(N2, 1) * sqrt(2/N);  % Hidden biases
w = randn(N2, 1) * sqrt(2/N);  % Output weights
...
%% Helper Functions
function [Psi, dPsi] = forward_pass(x, v, v2, theta, theta2, w, A)
    N = length(v);
    n_points = length(x);
    ...
    z2 = y * v2'  + theta2_broadcast; 
    y2 = sigmoid(z2);
    dy2dx = sigmoid_derivative(z2) .* (dydx * v2');
...
function [grad_v, grad_v2,grad_theta, grad_theta2, grad_w] = compute_gradients(x, v, v2, theta, theta2, w, target, A)
    ...
    for i = 1:N
        ...
        for j = 1:N2
            v2_plus = v2; v2_plus(j,i) = v2_plus(j,i) + epsilon;
            v2_minus = v2; v2_minus(j,i) = v2_minus(j,i) - epsilon;
    ...
    % Gradient for theta2
    for i = 1:N2
    theta2_plus = theta2; theta2_plus(i) = theta2_plus(i) + epsilon;
    theta2_minus = theta2; theta2_minus(i) = theta2_minus(i) - epsilon;
